...
Pipeline per volume:
  ...
  6. Save as subject_dir/ct.nii and subject_dir/mr.nii
...
Usage:
    ...
    python preprocess_mri.py --input_dir /path/to/niftis --output_dir /path/to/output --workers 8
...
def resize_volume(data: np.ndarray, height: int, width: int, order: int = 1):
...
# ---------------------------------------------------------------------------
# Single subject pipeline (top-level function for multiprocessing)
# ---------------------------------------------------------------------------

def preprocess_single(args_tuple):
    """
    Run the full preprocessing pipeline on a single NIfTI file.
    Takes a tuple for compatibility with ProcessPoolExecutor.
    """
    nii_path, output_dir, target_spacing, padding, clip_percentile, height, width = args_tuple
    ...
    # Reorient data+affine to RAS+ so axes are (R, A, S) = (x, y, slices)
    orig_ornt = nib.io_orientation(affine)
    ras_ornt = nib.orientations.axcodes2ornt(("R", "A", "S"))
    transform = nib.orientations.ornt_transform(orig_ornt, ras_ornt)
    stripped_data = nib.orientations.apply_orientation(stripped_data, transform)
    mask_data = nib.orientations.apply_orientation(mask_data, transform)
    affine = affine @ nib.orientations.inv_ornt_aff(transform, orig_img.shape[:3])
    ...
    # 4. Resize to target spatial dimensions (data + mask together to stay in sync)
    cropped_data = resize_volume(cropped_data, height, width)
    cropped_mask = resize_volume(cropped_mask.astype(np.float64), height, width, order=0) > 0.5

    # 5 & 6. Clip outliers and normalize to [0, 1]
    normalized_data = clip_and_normalize(cropped_data, cropped_mask, clip_percentile)

    # Save as ct.nii and mr.nii in subject directory
    ...
    return basename, normalized_data.shape, None


def preprocess_single_safe(args_tuple):
    """Wrapper that catches exceptions so one failure doesn't kill the pool."""
    try:
        return preprocess_single(args_tuple)
    except Exception as e:
        nii_path = args_tuple[0]
        basename = Path(nii_path).name.replace(".nii.gz", "").replace(".nii", "")
...
def main():
    ...
    task_args = [
        (nii_path, args.output_dir, args.target_spacing, args.padding, args.clip_percentile,
